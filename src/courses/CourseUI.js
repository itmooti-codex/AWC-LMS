...
export class CourseUI {
  ...
  static renderHomeItem(c) {
    ...
    if (role === 'teacher' || role === 'admin') {
      ...
      wrap.className = 'flex flex-col items-start gap-[24px] bg-white px-4 py-[24px]';
      ...
      if (start) {
        ...
        badge.className = 'flex items-center justify-center gap-2 rounded bg-[#ebf6f6] px-2 py-0.5';
  ...
  static renderList(list, container, type = 'nav') {
    if (!container) return;
    if (type !== 'home') {
      const html = (list || []).map(CourseUI.renderNavItem).join('') || '<div class="p-2 text-sm text-gray-500">No courses</div>';
      container.innerHTML = html;
      return;
    }
    const pool = CourseUI._ensurePool();
    const nextKeys = new Set((list || []).map(c => String(c.classUid || c.courseUid || c.id)));
